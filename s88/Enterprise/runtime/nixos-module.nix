...
let
  ...
  listenHost = runtimeNode.service.listenHost or (throw "FS-310-HDS-010-SDS-010-SMS-110: service.listenHost required for runtime node ${nodeName}");
...
in
{
  ...
  systemd.services.s88-runtime-tcpdump = {
    after = [ "network.target" ];
    wantedBy = [ "multi-user.target" ];
    serviceConfig = {
      ExecStart = "${pkgs.tcpdump}/bin/tcpdump -i any -nn -s 0 -U -w /run/s88-runtime-tcpdump/${nodeName}.pcap";
      RuntimeDirectory = "s88-runtime-tcpdump";
      Restart = "always";
      RestartSec = "2s";
      User = "root";
      Group = "root";
